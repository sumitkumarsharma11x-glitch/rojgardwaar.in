# -*- coding: utf-8 -*-
"""Validate P08 (physics chapter-08) question bank: structure + duplicate/stem-similarity
against all TWELVE locked biology banks + P01–P07 physics and within itself — plus an explicit
...
EXPECTED_TOPICS = {
    ...
    "reflection-and-mirrors": 50,
    "refraction-and-lenses": 50,
    "dispersion-and-human-eye": 50,
}
...
def main():
    ...
    # 1. count
    if len(p08) != 200:
        errors.append(f"count = {len(p08)} != 200")

    # 2. ids unique & sequential
    ids = [q["id"] for q in p08]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"phy8_q{n}" for n in range(101, 301)]:
        errors.append("id sequence not exactly phy8_q101..phy8_q300")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in p08 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(p08), 1), 1) for d, n in dc.items()}
    for d, exact in (("easy", 80), ("medium", 85), ("hard", 35)):
        if dc[d] != exact:
            errors.append(f"difficulty {d} = {dc[d]} != exact {exact}")

    # 6. GENERATION-ARTIFACT SCAN
    art = artifact_scan(p08, errors)
    ...
    check_cross("P08-internal", p08, p08, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", p08, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in p08 if q["correct"] == i) for i in range(4)}
    print(f"count={len(p08)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
