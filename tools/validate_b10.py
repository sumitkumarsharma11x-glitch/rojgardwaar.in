# -*- coding: utf-8 -*-
"""Validate B10 (bio chapter-10) question bank: structure + duplicate/stem-similarity
against all NINE locked banks (B01 ch-01, B09 ch-02, B02 ch-03, B03 ch-04, B04 ch-05,
B05 ch-06, B06 ch-07, B07 ch-08, B08 ch-09) and within itself — plus an explicit
...
EXPECTED_TOPICS = {
    "endocrine-system": 40,
    "puberty-adolescence": 30,
    "secondary-sexual-characteristics": 20,
    "growth-hormone": 20,
    "reproductive-phase": 20,
...
def main():
    ...
    # 1. count
    if len(b10) != 150:
        errors.append(f"count = {len(b10)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in b10]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"ch10_q{n}" for n in range(1001, 1151)]:
        errors.append("id sequence not exactly ch10_q1001..ch10_q1150")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in b10 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(b10), 1), 1) for d, n in dc.items()}
    ...
    art = artifact_scan(b10, errors)
    print(f"artifact-scan: {art} suspicious hit(s)")

    # 7. similarity: within B10, and vs all 9 locked banks
    ...
    check_cross("B10-internal", b10, b10, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", b10, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in b10 if q["correct"] == i) for i in range(4)}
    print(f"count={len(b10)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
