# -*- coding: utf-8 -*-
"""Validate B07 (bio chapter-08) question bank: structure + duplicate/stem-similarity
against locked banks B01 (chapter-01), B09 (chapter-02), B02 (chapter-03) and B03 (chapter-04), and within itself.
...
EXPECTED_TOPICS = {
    ...
    "aerobic-anaerobic-respiration": 30,
    "gas-exchange-transport": 30,
    "respiration-other-organisms": 30,
    "respiration-vs-other-processes": 20,
}
...
def main():
    ...
    # 1. count
    if len(b07) != 150:
        errors.append(f"count = {len(b07)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in b07]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"ch8_q{n}" for n in range(801, 951)]:
        errors.append("id sequence not exactly ch8_q801..ch8_q950")
    ...
    dc = {d: sum(1 for q in b07 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(b07), 1), 1) for d, n in dc.items()}
    ...
    check_cross("B07-internal", b07, b07, errors, warnings, internal=True)
    check_cross("vs-B01", b07, b01, errors, warnings)
    check_cross("vs-B09", b07, b09, errors, warnings)
    check_cross("vs-B02", b07, b02, errors, warnings)
    check_cross("vs-B03", b07, b03, errors, warnings)

    # report
    idx = {i: sum(1 for q in b07 if q["correct"] == i) for i in range(4)}
    print(f"count={len(b07)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
