...
against all TWELVE locked biology banks (B01 ch-01 ... B12 ch-12) and within itself — plus an explicit
...
def main():
    ...
    # 1. count
    if len(p01) != 150:
        errors.append(f"count = {len(p01)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in p01]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"phy1_q{n}" for n in range(101, 251)]:
        errors.append("id sequence not exactly phy1_q101..phy1_q250")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in p01 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(p01), 1), 1) for d, n in dc.items()}
    for d, exact in (("easy", 60), ("medium", 65), ("hard", 25)):
        if dc[d] != exact:
            errors.append(f"difficulty {d} = {dc[d]} != exact {exact}")

    # 6. GENERATION-ARTIFACT SCAN
    art = artifact_scan(p01, errors)
    ...
    check_cross("P01-internal", p01, p01, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", p01, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in p01 if q["correct"] == i) for i in range(4)}
    print(f"count={len(p01)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
