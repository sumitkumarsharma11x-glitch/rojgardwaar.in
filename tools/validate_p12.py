# -*- coding: utf-8 -*-
"""Validate P12 (physics chapter-12) question bank: structure + duplicate/stem-similarity
against all TWELVE locked biology banks + P01–P11 physics and within itself — plus an explicit
...
def main():
    ...
    # 1. count
    if len(p12) != 200:
        errors.append(f"count = {len(p12)} != 200")

    # 2. ids unique & sequential
    ids = [q["id"] for q in p12]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"phy12_q{n}" for n in range(101, 301)]:
        errors.append("id sequence not exactly phy12_q101..phy12_q300")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in p12 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(p12), 1), 1) for d, n in dc.items()}
    for d, exact in (("easy", 80), ("medium", 85), ("hard", 35)):
        if dc[d] != exact:
            errors.append(f"difficulty {d} = {dc[d]} != exact {exact}")

    # 6. GENERATION-ARTIFACT SCAN
    art = artifact_scan(p12, errors)
    ...
    check_cross("P12-internal", p12, p12, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", p12, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in p12 if q["correct"] == i) for i in range(4)}
    print(f"count={len(p12)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
