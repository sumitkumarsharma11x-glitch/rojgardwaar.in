# -*- coding: utf-8 -*-
"""Validate C02 (chemistry chapter-02) question bank: structure + duplicate/stem-similarity
against all TWELVE biology banks + P01-P12 physics + C01 chemistry (25 frozen chapters) and within
itself — plus an explicit GENERATION-ARTIFACT SCAN (stray trailing numbers, placeholders,
markers, template fragments, duplicate punctuation, unfinished sentences) on every text field.
...
def main():
    ...
    c01 = load(os.path.join(ROOT, "data/science/chemistry/chapter-02/questions.json"))
    banks = {
        "B01": load(os.path.join(ROOT, "data/science/biology/chapter-02/questions.json")),
        ...
        "P01": load(os.path.join(ROOT, "data/science/physics/chapter-02/questions.json")),
        "P02": load(os.path.join(ROOT, "data/science/physics/chapter-02/questions.json")),
        "P03": load(os.path.join(ROOT, "data/science/physics/chapter-03/questions.json")),
        "P04": load(os.path.join(ROOT, "data/science/physics/chapter-04/questions.json")),
        "P05": load(os.path.join(ROOT, "data/science/physics/chapter-05/questions.json")),
        "P06": load(os.path.join(ROOT, "data/science/physics/chapter-06/questions.json")),
        "P07": load(os.path.join(ROOT, "data/science/physics/chapter-07/questions.json")),
        "P08": load(os.path.join(ROOT, "data/science/physics/chapter-08/questions.json")),
        "P09": load(os.path.join(ROOT, "data/science/physics/chapter-09/questions.json")),
        "P10": load(os.path.join(ROOT, "data/science/physics/chapter-10/questions.json")),
        "P11": load(os.path.join(ROOT, "data/science/physics/chapter-11/questions.json")),
        "P12": load(os.path.join(ROOT, "data/science/physics/chapter-12/questions.json")),
        "C01": load(os.path.join(ROOT, "data/science/chemistry/chapter-01/questions.json")),
    }

    # 1. count
    if len(c01) != 200:
        errors.append(f"count = {len(c01)} != 200")

    # 2. ids unique & sequential
    ids = [q["id"] for q in c01]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"chem2_q{n}" for n in range(101, 301)]:
        errors.append("id sequence not exactly chem2_q101..chem2_q300")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in c01 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(c01), 1), 1) for d, n in dc.items()}
    for d, exact in (("easy", 80), ("medium", 85), ("hard", 35)):
        if dc[d] != exact:
            errors.append(f"difficulty {d} = {dc[d]} != exact {exact}")

    # 6. GENERATION-ARTIFACT SCAN
    art = artifact_scan(c01, errors)
    print(f"artifact-scan: {art} suspicious hit(s)")

    # 7. similarity: within C01, and vs all 24 locked banks
    ...
    check_cross("C02-internal", c01, c01, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", c01, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in c01 if q["correct"] == i) for i in range(4)}
    print(f"count={len(c01)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
