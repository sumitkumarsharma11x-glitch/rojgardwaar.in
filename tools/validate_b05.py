# -*- coding: utf-8 -*-
"""Validate B05 (bio chapter-06) question bank: structure + duplicate/stem-similarity
against locked banks B01 (ch-01), B09 (ch-02), B02 (ch-03), B03 (ch-04), B04 (ch-05),
B06 (ch-07), B07 (ch-08) and B08 (ch-09), within itself — plus an explicit GENERATION-
ARTIFACT SCAN (stray trailing numbers, placeholders, markers, duplicated punctuation,
unfinished text) on every text field of every question, as demanded for this batch.
...
def main():
    ...
    # 1. count
    if len(b05) != 150:
        errors.append(f"count = {len(b05)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in b05]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"ch6_q{n}" for n in range(601, 751)]:
        errors.append("id sequence not exactly ch6_q601..ch6_q750")
    ...
    # 4. per-question structure (7 keys exactly, no extra metadata)
    ...
    # 5. difficulty mix (easy 30-35%, medium 50-60%, hard 10-15%)
    dc = {d: sum(1 for q in b05 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(b05), 1), 1) for d, n in dc.items()}
    for d, lo, hi_ in (("easy", 30, 35), ("medium", 50, 60), ("hard", 10, 15)):
        if not lo <= pct[d] <= hi_:
            errors.append(f"difficulty {d} = {pct[d]}% outside {lo}-{hi}%")

    # 6. GENERATION-ARTIFACT SCAN (explicit, all fields)
    art = artifact_scan(b05, errors)
    print(f"artifact-scan: {art} suspicious hit(s)")

    # 7. similarity: within B05, and vs all 8 locked banks
    ...
    check_cross("B05-internal", b05, b05, errors, warnings, internal=True)
    check_cross("vs-B01", b05, b01, errors, warnings)
    check_cross("vs-B09", b05, b09, errors, warnings)
    check_cross("vs-B02", b05, b02, errors, warnings)
    check_cross("vs-B03", b05, b03, errors, warnings)
    check_cross("vs-B04", b05, b04, errors, warnings)
    check_cross("vs-B06", b05, b06, errors, warnings)
    check_cross("vs-B07", b05, b07, errors, warnings)
    check_cross("vs-B08", b05, b08, errors, warnings)

    # report
    idx = {i: sum(1 for q in b05 if q["correct"] == i) for i in range(4)}
    print(f"count={len(b05)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
