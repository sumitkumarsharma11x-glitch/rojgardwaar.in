# -*- coding: utf-8 -*-
"""Validate P04 (physics chapter-04) question bank: structure + duplicate/stem-similarity
against all TWELVE locked biology banks + P01, P02, P03 physics and within itself — plus an explicit
...
def main():
    ...
    # 1. count
    if len(p04) != 150:
        errors.append(f"count = {len(p04)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in p04]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"phy4_q{n}" for n in range(101, 251)]:
        errors.append("id sequence not exactly phy4_q101..phy4_q250")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in p04 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(p04), 1), 1) for d, n in dc.items()}
    for d, exact in (("easy", 60), ("medium", 65), ("hard", 25)):
        if dc[d] != exact:
            errors.append(f"difficulty {d} = {dc[d]} != exact {exact}")

    # 6. GENERATION-ARTIFACT SCAN
    art = artifact_scan(p04, errors)
    ...
    check_cross("P04-internal", p04, p04, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", p04, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in p04 if q["correct"] == i) for i in range(4)}
    print(f"count={len(p04)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
