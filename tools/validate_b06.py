# -*- coding: utf-8 -*-
"""Validate B06 (bio chapter-07) question bank: structure + duplicate/stem-similarity
against locked banks B01 (chapter-01), B09 (chapter-02), B02 (chapter-03), B03 (chapter-04),
B04 (chapter-05), B07 (chapter-08) and B08 (chapter-09), and within itself.
...
EXPECTED_TOPICS = {
    "nutrients-balanced-diet": 35,
    "human-digestive-system": 35,
    "enzymes-digestion-steps": 30,
    "nutrition-modes": 20,
    "ingestion-other-animals": 15,
...
def main():
    ...
    # 1. count
    if len(b06) != 150:
        errors.append(f"count = {len(b06)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in b06]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"ch7_q{n}" for n in range(701, 851)]:
        errors.append("id sequence not exactly ch7_q701..ch7_q850")
    ...
    dc = {d: sum(1 for q in b06 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(b06), 1), 1) for d, n in dc.items()}
    for d, lo, hi_ in (("easy", 30, 35), ("medium", 50, 60), ("hard", 10, 15)):
        if not lo <= pct[d] <= hi_:
            errors.append(f"difficulty {d} = {pct[d]}% outside {lo}-{hi}%")

    # 6. similarity: within B06, and vs locked banks B01/B09/B02/B03/B04/B07/B08
    ...
    check_cross("B06-internal", b06, b06, errors, warnings, internal=True)
    check_cross("vs-B01", b06, b01, errors, warnings)
    check_cross("vs-B09", b06, b09, errors, warnings)
    check_cross("vs-B02", b06, b02, errors, warnings)
    check_cross("vs-B03", b06, b03, errors, warnings)
    check_cross("vs-B04", b06, b04, errors, warnings)
    check_cross("vs-B07", b06, b07, errors, warnings)
    check_cross("vs-B08", b06, b08, errors, warnings)

    # report
    idx = {i: sum(1 for q in b06 if q["correct"] == i) for i in range(4)}
    print(f"count={len(b06)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
