# -*- coding: utf-8 -*-
"""Validate P03 (physics chapter-03) question bank: structure + duplicate/stem-similarity
against all TWELVE locked biology banks + P01, P02 physics and within itself — plus an explicit
...
EXPECTED_TOPICS = {
    "force-and-effects": 38,
    "law-of-inertia": 38,
    "momentum-second-law": 37,
    "action-reaction-friction": 37,
}
...
def main():
    ...
    # 1. count
    if len(p03) != 150:
        errors.append(f"count = {len(p03)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in p03]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"phy3_q{n}" for n in range(101, 251)]:
        errors.append("id sequence not exactly phy3_q101..phy3_q250")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in p03 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(p03), 1), 1) for d, n in dc.items()}
    for d, exact in (("easy", 60), ("medium", 65), ("hard", 25)):
        if dc[d] != exact:
            errors.append(f"difficulty {d} = {dc[d]} != exact {exact}")

    # 6. GENERATION-ARTIFACT SCAN
    art = artifact_scan(p03, errors)
    ...
    check_cross("P03-internal", p03, p03, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", p03, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in p03 if q["correct"] == i) for i in range(4)}
    print(f"count={len(p03)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
