# -*- coding: utf-8 -*-
"""Validate P05 (physics chapter-05) question bank: structure + duplicate/stem-similarity
against all TWELVE locked biology banks + P01–P04 physics and within itself — plus an explicit
...
EXPECTED_TOPICS = {
    "work": 38,
    "kinetic-energy": 38,
    "potential-energy-conservation": 37,
    "power": 37,
}
...
def main():
    ...
    # 1. count
    if len(p05) != 150:
        errors.append(f"count = {len(p05)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in p05]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"phy5_q{n}" for n in range(101, 251)]:
        errors.append("id sequence not exactly phy5_q101..phy5_q250")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in p05 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(p05), 1), 1) for d, n in dc.items()}
    for d, exact in (("easy", 60), ("medium", 65), ("hard", 25)):
        if dc[d] != exact:
            errors.append(f"difficulty {d} = {dc[d]} != exact {exact}")

    # 6. GENERATION-ARTIFACT SCAN
    art = artifact_scan(p05, errors)
    ...
    check_cross("P05-internal", p05, p05, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", p05, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in p05 if q["correct"] == i) for i in range(4)}
    print(f"count={len(p05)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
