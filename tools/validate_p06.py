# -*- coding: utf-8 -*-
"""Validate P06 (physics chapter-06) question bank: structure + duplicate/stem-similarity
against all TWELVE locked biology banks + P01–P05 physics and within itself — plus an explicit
...
def main():
    ...
    # 1. count
    if len(p06) != 150:
        errors.append(f"count = {len(p06)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in p06]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"phy6_q{n}" for n in range(101, 251)]:
        errors.append("id sequence not exactly phy6_q101..phy6_q250")
    ...
    # 5. difficulty mix
    dc = {d: sum(1 for q in p06 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(p06), 1), 1) for d, n in dc.items()}
    for d, exact in (("easy", 60), ("medium", 65), ("hard", 25)):
        if dc[d] != exact:
            errors.append(f"difficulty {d} = {dc[d]} != exact {exact}")

    # 6. GENERATION-ARTIFACT SCAN
    art = artifact_scan(p06, errors)
    ...
    check_cross("P06-internal", p06, p06, errors, warnings, internal=True)
    for name, bank in banks.items():
        check_cross(f"vs-{name}", p06, bank, errors, warnings)

    # report
    idx = {i: sum(1 for q in p06 if q["correct"] == i) for i in range(4)}
    print(f"count={len(p06)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
