# -*- coding: utf-8 -*-
"""Validate B02 (bio chapter-03) question bank: structure + duplicate/stem-similarity
against locked banks B01 (chapter-01) and B09 (chapter-02), and within itself.
...
EXPECTED_TOPICS = {
    ...
    "epithelial-connective-tissues": 25,
    "joints-movement": 25,
...
def main():
    ...
    # 1. count
    if len(b02) != 150:
        errors.append(f"count = {len(b02)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in b02]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"ch3_q{n}" for n in range(301, 451)]:
        errors.append("id sequence not exactly ch3_q301..ch3_q450")
    ...
    dc = {d: sum(1 for q in b02 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(b02), 1), 1) for d, n in dc.items()}
    ...
    check_cross("B02-internal", b02, b02, errors, warnings, internal=True)
    check_cross("vs-B01", b02, b01, errors, warnings)
    check_cross("vs-B09", b02, b09, errors, warnings)

    # report
    idx = {i: sum(1 for q in b02 if q["correct"] == i) for i in range(4)}
    print(f"count={len(b02)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
