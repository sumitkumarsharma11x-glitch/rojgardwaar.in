# -*- coding: utf-8 -*-
"""Validate B08 (bio chapter-09) question bank: structure + duplicate/stem-similarity
against locked banks B01 (chapter-01), B09 (chapter-02), B02 (chapter-03), B03 (chapter-04)
and B07 (chapter-08), and within itself.
...
EXPECTED_TOPICS = {
    "heart": 35,
    "blood-components": 30,
    "blood-vessels-pressure": 25,
    "xylem-transport": 25,
    "double-circulation-lymph": 20,
    "phloem-translocation": 15,
}
...
def main():
    ...
    # 1. count
    if len(b08) != 150:
        errors.append(f"count = {len(b08)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in b08]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"ch9_q{n}" for n in range(901, 1051)]:
        errors.append("id sequence not exactly ch9_q901..ch9_q1050")
    ...
    dc = {d: sum(1 for q in b08 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(b08), 1), 1) for d, n in dc.items()}
    for d, lo, hi_ in (("easy", 30, 35), ("medium", 50, 60), ("hard", 10, 15)):
        if not lo <= pct[d] <= hi_:
            errors.append(f"difficulty {d} = {pct[d]}% outside {lo}-{hi}%")

    # 6. similarity: within B08, and vs locked banks B01/B09/B02/B03/B07
    ...
    check_cross("B08-internal", b08, b08, errors, warnings, internal=True)
    check_cross("vs-B01", b08, b01, errors, warnings)
    check_cross("vs-B09", b08, b09, errors, warnings)
    check_cross("vs-B02", b08, b02, errors, warnings)
    check_cross("vs-B03", b08, b03, errors, warnings)
    check_cross("vs-B07", b08, b07, errors, warnings)

    # report
    idx = {i: sum(1 for q in b08 if q["correct"] == i) for i in range(4)}
    print(f"count={len(b08)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
