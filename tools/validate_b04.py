# -*- coding: utf-8 -*-
"""Validate B04 (bio chapter-05) question bank: structure + duplicate/stem-similarity
against locked banks B01 (chapter-01), B09 (chapter-02), B02 (chapter-03), B03 (chapter-04),
B07 (chapter-08) and B08 (chapter-09), and within itself.
...
EXPECTED_TOPICS = {
    "photosynthesis": 35,
    "nutrition-modes-in-plants": 30,
    "plant-parts-functions": 25,
    "crop-production": 40,
    "photosynthesis-experiments": 20,
}
...
def main():
    ...
    # 1. count
    if len(b04) != 150:
        errors.append(f"count = {len(b04)} != 150")

    # 2. ids unique & sequential
    ids = [q["id"] for q in b04]
    if len(set(ids)) != len(ids):
        errors.append("duplicate ids")
    if ids != [f"ch5_q{n}" for n in range(501, 651)]:
        errors.append("id sequence not exactly ch5_q501..ch5_q650")
    ...
    dc = {d: sum(1 for q in b04 if q["difficulty"] == d) for d in ("easy", "medium", "hard")}
    pct = {d: round(100 * n / max(len(b04), 1), 1) for d, n in dc.items()}
    for d, lo, hi_ in (("easy", 30, 35), ("medium", 50, 60), ("hard", 10, 15)):
        if not lo <= pct[d] <= hi_:
            errors.append(f"difficulty {d} = {pct[d]}% outside {lo}-{hi}%")

    # 6. similarity: within B04, and vs locked banks B01/B09/B02/B03/B07/B08
    ...
    check_cross("B04-internal", b04, b04, errors, warnings, internal=True)
    check_cross("vs-B01", b04, b01, errors, warnings)
    check_cross("vs-B09", b04, b09, errors, warnings)
    check_cross("vs-B02", b04, b02, errors, warnings)
    check_cross("vs-B03", b04, b03, errors, warnings)
    check_cross("vs-B07", b04, b07, errors, warnings)
    check_cross("vs-B08", b04, b08, errors, warnings)

    # report
    idx = {i: sum(1 for q in b04 if q["correct"] == i) for i in range(4)}
    print(f"count={len(b04)}  difficulty={dc} ({pct}%)  correct-index={idx}")
    print(f"topics={tc}")
    if warnings:
